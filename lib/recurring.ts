...
export async function updateRecurringSchedule(id: string, input: Partial<RecurringInput> & { status?: string; ends_on?: string | null }) {
  ...
  // The current UI represents a multi-day recurrence as sibling rows. Retire
  // those siblings before the caller recreates exactly the selected weekdays.
  if (current.status === "active" && input.status !== "cancelled") {
    let q = database.from("recurring_schedules").update({ status: "cancelled" }).eq("teacher_id", teacher_id).eq("status", "active").neq("id", id).eq("title", current.title).eq("start_time", current.start_time).eq("timezone", current.timezone);
