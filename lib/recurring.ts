...
export async function createRecurringSchedule(input: RecurringInput) {
  ...
  const { data: existing, error: lookupError } = input.student_id
    ? await query.eq("student_id", input.student_id).limit(1).maybeSingle()
    : await query.is("student_id", null).limit(1).maybeSingle();
  ...
  return database
    .from("recurring_schedules")
    .insert({ ...input, teacher_id, status: "active" })
    .select()
    .single();
}
...
export async function updateRecurringSchedule(
  id: string,
  input: Partial<RecurringInput> & { status?: string; ends_on?: string | null }
) {
  const teacher_id = await requireTeacherId();
  return db()
    .from("recurring_schedules")
...
export async function syncRecurringSchedules(days = 90) {
  const teacher_id = await requireTeacherId();
  const database = db();
  const { data: schedules, error } = await database
    .from("recurring_schedules")
    .select("*")
    .eq("teacher_id", teacher_id)
    .eq("status", "active");
  ...
  const existingKeys = new Set(
    (existing || []).map((e: any) => `${e.recurring_schedule_id}|${e.starts_at}`)
  );
  const inserts: any[] = [];

  for (const schedule of schedules as any[]) {
    const first = new Date(`${schedule.starts_on}T00:00:00`);
    const startDate = new Date(Math.max(today.getTime(), first.getTime()));
    const scheduleEnd = schedule.ends_on
      ? new Date(`${schedule.ends_on}T23:59:59`)
      : null;
    ...
    for (
      ...
      if (scheduleEnd && cursor > scheduleEnd) break;
      ...
      const end = new Date(
        start.getTime() + Number(schedule.duration_minutes || 60) * 60000
      );
      const startsAt = start.toISOString();
      const key = `${schedule.id}|${startsAt}`;
      ...
      if (!existingKeys.has(key)) {
        ...
        inserts.push({
          ...
          event_type: schedule.student_id ? "lesson" : "personal",
  ...
  return database
    .from("events")
    .upsert(inserts, {
      onConflict: "recurring_schedule_id,starts_at",
