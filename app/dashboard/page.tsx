...
import { AlertTriangle, ArrowLeft, BookOpen, Calendar, CheckCircle2, Clock, Compass, MessageCircle, Play, Sparkles, TrendingUp, Users, Wallet, Globe2 } from "lucide-react";
import AppShell from "../../components/app-shell";
import { getEvents, getStudents } from "../../lib/data";
import { formatInTimeZone, formatTimeInZone } from "../../lib/timezone";
import "./dashboard.css";

type Student={id:string;full_name:string;age:number|null;timezone:string;status:string;monthly_hours?:number;meeting_link?:string|null;country_code?:string|null};
...
function countryFlag(code?:string|null, timezone?:string){
 const c=(code||"").toUpperCase();
 const tz=timezone||"";
 const map:Record<string,string>={EG:"🇪🇬",BE:"🇧🇪",FR:"🇫🇷",NL:"🇳🇱",MA:"🇲🇦",DZ:"🇩🇿",TN:"🇹🇳",SA:"🇸🇦",AE:"🇦🇪",QA:"🇶🇦",KW:"🇰🇼",GB:"🇬🇧",US:"🇺🇸",CA:"🇨🇦",DE:"🇩🇪",IT:"🇮🇹",ES:"🇪🇸",TR:"🇹🇷"};
 if(map[c]) return map[c];
 if(tz.startsWith("Africa/Cairo")) return "🇪🇬";
 if(tz.startsWith("Europe/Brussels")) return "🇧🇪";
 if(tz.startsWith("Europe/Paris")) return "🇫🇷";
 if(tz.startsWith("Europe/Amsterdam")) return "🇳🇱";
 if(tz.startsWith("Africa/Casablanca")) return "🇲🇦";
 if(tz.startsWith("Africa/Algiers")) return "🇩🇿";
 if(tz.startsWith("Africa/Tunis")) return "🇹🇳";
 if(tz.startsWith("Asia/Riyadh")) return "🇸🇦";
 if(tz.startsWith("Asia/Dubai")) return "🇦🇪";
 if(tz.startsWith("Europe/London")) return "🇬🇧";
 return "🌍";
}
function countryName(code?:string|null){const names:Record<string,string>={EG:"مصر",BE:"بلجيكا",FR:"فرنسا",NL:"هولندا",MA:"المغرب",DZ:"الجزائر",TN:"تونس",SA:"السعودية",AE:"الإمارات",QA:"قطر",KW:"الكويت",GB:"بريطانيا",US:"الولايات المتحدة",CA:"كندا",DE:"ألمانيا",IT:"إيطاليا",ES:"إسبانيا",TR:"تركيا"};return code?names[code.toUpperCase()]||code.toUpperCase():""}
...
export default function DashboardPage(){
 const [students,setStudents]=useState<Student[]>([]),[events,setEvents]=useState<Event[]>([]),[loading,setLoading]=useState(true),[now,setNow]=useState(new Date());
 ...
 const teacherTimezone=Intl.DateTimeFormat().resolvedOptions().timeZone||"UTC", teacherFlag=countryFlag(null,teacherTimezone), teacherTime=now.toLocaleTimeString("ar-EG",{hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:true});
 const nextStudent=next?students.find(s=>s.id===next.student_id):undefined;
 const studentTimezone=next?.timezone||nextStudent?.timezone||"UTC", studentTime=next?new Intl.DateTimeFormat("ar-EG",{timeZone:studentTimezone,hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:true}).format(now):"—";
 const timezoneOffset=(tz:string)=>{const d=new Date(),parts=new Intl.DateTimeFormat("en-US",{timeZone:tz,timeZoneName:"longOffset"}).formatToParts(d),v=parts.find(p=>p.type==="timeZoneName")?.value||"GMT";return v.replace("GMT","")||"+00:00"};
 const diffMinutes=(a:string,b:string)=>{const get=(tz:string)=>{const p=new Intl.DateTimeFormat("en-US",{timeZone:tz,hour12:false,hour:"2-digit",minute:"2-digit"}).formatToParts(now);return Number(p.find(x=>x.type==="hour")?.value||0)*60+Number(p.find(x=>x.type==="minute")?.value||0)};let diff=get(a)-get(b);if(diff>720)diff-=1440;if(diff<-720)diff+=1440;return Math.round(diff/60)};
 const studentDiff=next?diffMinutes(studentTimezone,teacherTimezone):0;
 return <AppShell><main className="dashboard-page">
  ...
  <div className="riwaq-dashboard-grid">
   ...
   <aside className="card riwaq-summary-card"><div className="riwaq-section-head"><div><span className="riwaq-section-kicker"><Compass size={14}/> ملخص سريع</span><h2>اليوم</h2></div></div><div className="riwaq-summary-list"><div><span>إجمالي الأنشطة</span><b>{today.length}</b></div><div><span>المواعيد القادمة</span><b>{today.filter(e=>+new Date(e.starts_at)>Date.now()).length}</b></div><div><span>الساعات المخططة</span><b>{today.reduce((n,e)=>n+hours(e),0).toFixed(1)}</b></div><div><span>التنبيهات</span><b>0</b></div></div><Link href="/calendar-riwaq" className="riwaq-more">فتح الجدول الكامل <ArrowLeft size={14}/></Link></aside>
  </div>
  ...
  <section className="card riwaq-timezone-card"><div className="riwaq-section-head"><div><span className="riwaq-section-kicker"><Globe2 size={14}/> التوقيت العالمي</span><h2>فرق التوقيت بين المعلم والطلاب</h2></div></div><div className="riwaq-clocks"><div className="riwaq-clock"><div className="riwaq-clock-top"><span className="riwaq-flag">{teacherFlag}</span><span>توقيت المعلم</span></div><div className="riwaq-clock-time">{teacherTime}</div><div className="riwaq-clock-zone">{teacherTimezone} · UTC{timezoneOffset(teacherTimezone)}</div></div><div className="riwaq-clock"><div className="riwaq-clock-top"><span className="riwaq-flag">{next?countryFlag(nextStudent?.country_code,studentTimezone):"🌍"}</span><span>{nextStudent?.full_name||"الطالب القادم"}</span></div><div className="riwaq-clock-time">{studentTime}</div><div className="riwaq-clock-zone">{studentTimezone} · UTC{timezoneOffset(studentTimezone)}{next&&studentDiff!==0?` · ${studentDiff>0?`متقدم ${studentDiff} ساعة`:`متأخر ${Math.abs(studentDiff)} ساعة`}`:""}</div></div></div></section>

  <section className="card riwaq-students-card"><div className="riwaq-section-head"><div><span className="riwaq-section-kicker"><Users size={14}/> الطلاب</span><h2>الطلاب النشطون</h2></div><Link href="/students" className="riwaq-more">إدارة الطلاب <ArrowLeft size={14}/></Link></div><div className="riwaq-student-grid">{active.slice(0,8).map(s=><Link href={`/students/${s.id}`} className="riwaq-student" key={s.id}><div className="riwaq-avatar">{s.full_name?.trim()?.charAt(0)||"ط"}</div><div><b>{s.full_name}</b><span className="riwaq-student-meta"><span className="riwaq-flag">{countryFlag(s.country_code,s.timezone)}</span>{countryName(s.country_code)||s.timezone}</span></div><ArrowLeft size={14}/></Link>)}</div></section>
 </main></AppShell>;
}
