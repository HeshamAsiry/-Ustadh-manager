  ...
  return <main className="dashboard-shell" dir="rtl">
    ...
    <section className="dashboard-content">
      ...
      <section className="section-block"><div className="section-heading"><div><span className="section-index">04</span><h2>إحصائيات وإنجازات الشهر الحالي</h2></div><a href="/reports">عرض التقارير الموسعة <ChevronLeft size={15}/></a></div>
        <div className="month-card"><div className="month-head"><div><span>الشهر الحالي</span><h3>{monthLabel}</h3></div><a href="/reports">التفاصيل <ArrowLeft size={15}/></a></div><div className="month-stats"><div><strong>{monthTotalHours.toFixed(1)}</strong><span>إجمالي ساعات التدريس المسجلة</span></div><div><strong>{monthCompleted}</strong><span>الحصص المكتملة</span></div><div><strong>{students.length}</strong><span>الطلاب المسجلون</span></div><div><strong>—</strong><span>متوسط الحضور</span></div></div><div className="progress-note"><span>تُحسب الساعات من الحصص المكتملة المسجلة في قاعدة البيانات.</span><a href="/lessons"><Plus size={15}/> تسجيل حصة</a></div></div>
      </section>
