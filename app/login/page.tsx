...
export default function LoginPage(){
  ...
  return <main className="auth-page"><section className="auth-card">
    ...
    <div className="tabs"><button className={`tab ${mode==="login"?"active":""}`} onClick={()=>{setMode("login");setError("");setMessage("")}}>تسجيل الدخول</button><button className={`tab ${mode==="signup"?"active":""}`} onClick={()=>{setMode("signup");setError("");setMessage("")}}>إنشاء حساب</button></div>
    <form className="form" onSubmit={submit}>
      ...
      <button className="submit" disabled={busy}>{busy?(mode==="login"?"جارٍ تسجيل الدخول...":"جارٍ إنشاء الحساب..."):(mode==="login"?"تسجيل الدخول":"إنشاء الحساب")}</button>
