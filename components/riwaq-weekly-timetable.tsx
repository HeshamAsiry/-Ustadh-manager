...
export type RiwaqStudent = { id: string; full_name: string; timezone: string; country?: string | null; countryFlag?: string | null; country_code?: string | null };
 ...
 return <div className="space-y-6" dir="rtl">
  <div className="calendar-toolbar card" style={{marginBottom:0}}>
   ...
   <div className="top-actions">
    <label className="filter-select" style={{display:"flex",alignItems:"center",gap:7}}><FilterIcon/><select value={studentFilter} onChange={e=>setStudentFilter(e.target.value)}><option value="all">جميع الطلاب ({students.length})</option>{students.map(s=><option key={s.id} value={s.id}>{countryFlag(s.country_code) || s.countryFlag || ""} {s.full_name}</option>)}</select></label>
