          ...
          <div className="te-day-body">
            {dayEvents.length === 0 ? <div className="te-empty-day">لا توجد مواعيد مسجلة في هذا اليوم</div> : dayEvents.map(event => {
              ...
              return <article className={`te-event status-${event.status}`} key={event.id}>
                ...
                <div className="student-card-head" style={{ marginBottom: 8 }}><div style={{ minWidth: 0 }}><strong style={{ display: "block", fontSize: 13 }}>{student.countryFlag || ""} {student.full_name}</strong><span style={{ fontSize: 10, color: "var(--muted)" }}>{student.country || ""}</span></div><div className="top-actions" style={{ gap: 2 }}><button className="icon-btn" title="واتساب" onClick={() => onWhatsApp(event, student)}><MessageSquare size={14} /></button><button className="icon-btn" title="تعديل" onClick={() => onEdit(event)}><Pencil size={14} /></button><button className="icon-btn danger-icon" title="حذف" onClick={() => onDelete(event)}><Trash2 size={14} /></button></div></div>
