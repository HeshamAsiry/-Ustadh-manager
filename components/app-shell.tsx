...
const DASHBOARD_LOGO = "https://raw.githubusercontent.com/HeshamAsiry/-Ustadh-manager/main/dashboard%20logo.png";

export default function AppShell({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  const [mobileOpen, setMobileOpen] = useState(false);
  async function logout() { await supabase?.auth.signOut(); window.location.replace("/login"); }
  const Brand = ({mobile=false}:{mobile?:boolean}) => <div className={mobile ? "brand brand-image mobile-brand" : "brand brand-image"}><img src={DASHBOARD_LOGO} alt="Ustadh Manager" /><div><span>Ustadh Manager</span><small>إدارة التدريس والطلاب</small></div></div>;
