create table if not exists public.payments (
  ...
  check (
    (status='paid' and amount_paid >= amount)
    or (status='partial' and amount_paid > 0 and amount_paid < amount)
...
create unique index if not exists payments_teacher_student_month_uidx
  on public.payments (
    teacher_id,
    coalesce(student_id::text,'legacy:'||coalesce(legacy_student_id,'')),
    coalesce(month_year,'')
  );
...
for each row execute function public.set_updated_at();
  ...
  check (month_year is null or month_year ~ '^[0-9]{4}-(0[1-9]|1[0-2])
  check (abs(total_due-greatest(0,amount-amount_paid))<=0.01)
);
