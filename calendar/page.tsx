...
export default function CalendarPage({scope="lessons"}:{scope?: "lessons"|"personal"|"all"}){
  ...
  return <main className="calendar-shell" dir="rtl">
    ...
    {conflicts.length>0&&<button className="conflict-alert" onClick={()=>setView("day")}><span><AlertTriangle size={19}/></span><div><strong>تم اكتشاف تعارض في البيانات المعروضة.</strong><small>يظهر التعارض عندما يتداخل موعد مسجل مع موعد آخر أو مع قاعدة أسبوعية؛ المواعيد الجديدة تُفحص أيضًا قبل الحفظ.</small></div><ChevronLeft size={18}/></button>}
    ...
    {!loading&&view==="week"&&<section className="week-grid">
      {weekDates.map((date,i)=>{const key=dateKey(date),items=visibleAppointments.filter(a=>a.date===key).sort((a,b)=>minutes(a.time)-minutes(b.time));return <div className={"day-column "+(selectedDate===key?"selected":"")} key={key}>
        ...
        <div className="day-list">{items.map(a=><article className={"appointment-card "+(conflictIds.has(a.id)?"appointment-conflict":"")} key={a.id}>
          ...
          {menuId===a.id&&<div className="appointment-menu">{a.isRecurring?<button onClick={()=>{setMenuId(null);setMessage("هذا موعد ناتج عن قاعدة أسبوعية. عطّل قاعدة التكرار من إعدادات الجدول بدل إلغائه كموعد منفرد.")}}>متكرر أسبوعيًا</button>:<><button onClick={()=>openEdit(a)}>تعديل</button>{a.eventType==="personal"&&<a className="calendar-google-link" href={googleUrlFor(a)} target="_blank" rel="noreferrer"><ExternalLink size={15}/>تقويم Google</a>}<button onClick={()=>deleteAppointment(a.id)}>إلغاء الموعد</button></>}</div>}
        </article>)}</div>
    ...
    <footer className="calendar-footer"><span><Clock3 size={15}/> جميع الأوقات الأساسية معروضة بتوقيت المعلم: {teacherTimezone}</span><span><CalendarDays size={15}/> المواعيد الجديدة تُحفظ مباشرة في قاعدة البيانات، والقواعد الأسبوعية تظهر كوقوعات متكررة.</span></footer>
