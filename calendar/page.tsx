  ...
  const load=async()=>{
    ...
    if(eventResult.error){setMessage(eventResult.error.message);setAppointments([]);}
      ...
      setAppointments((eventResult.data||[]).map((e:any)=>{
        ...
        return {id:e.id,date:parts.date,time:parts.time,end:endParts.time,studentId:e.student_id,studentIds:ids,student:names.length>1?names.join("، "):(names[0]||"طالب محذوف"),country:countryName(code),countryCode:code,timezone:primary?.timezone||"Africa/Cairo",subject:e.title||"القرآن الكريم",duration,status:e.status};
      }));
    }
    setLoading(false);
  };
