...
export default function CalendarPage(){
  ...
  const load=async()=>{
    ...
    if(eventResult.error){setMessage(eventResult.error.message);setAppointments([]);}
      ...
      setAppointments((eventResult.data||[]).map((e:any)=>{
        const ids=(participantsByEvent[e.id]&&participantsByEvent[e.id].length?participantsByEvent[e.id]:[e.student_id]).filter(Boolean) as string[];
        const names=ids.map(id=>byId[id]?.full_name).filter(Boolean) as string[];
        ...
        return {id:e.id,date:parts.date,time:parts.time,end:endParts.time,studentId:e.student_id||ids[0]||"",studentIds:ids,student:names.length>1?names.join("، "):(names[0]||"طالب محذوف"),country:countryName(code),countryCode:code,timezone:primary?.timezone||"Africa/Cairo",subject:e.title||"القرآن الكريم",duration,status:e.status};
      }));
    }
    setLoading(false);
  };
  ...
  const openAdd=(date=selectedDate,time="18:00")=>{
    ...
    setForm({...emptyForm(date),time,studentId:firstStudent});
    setModalOpen(true);
  };
  const openEdit=(a:Appointment)=>{
    setEditingId(a.id);
    setForm({date:a.date,time:a.time,studentId:a.studentId,subject:a.subject,duration:a.duration,status:a.status});
    setModalOpen(true);
    setMenuId(null);
  };
  ...
  const saveAppointment=async()=>{
    ...
    if(!student){setMessage("اختر طالبًا أولًا.");return}
    ...
    const payload={
      ...
      event_type:"lesson",
      title:form.subject+" — "+student.full_name,
  ...
  return <main className="calendar-shell" dir="rtl">
    <header className="calendar-header">
      <div><span className="calendar-eyebrow">إدارة المواعيد</span><h1>التقويم والمواعيد</h1><p>المواعيد محفوظة في قاعدة البيانات، وتُعرض بتوقيت المعلم مع تحويل وقت الطالب تلقائيًا.</p></div>
      <button className="add-appointment" onClick={()=>openAdd()} disabled={loading||students.length===0}><Plus size={18}/> إضافة موعد</button>
    </header>
    ...
    {!loading&&view==="week"&&<section className="week-grid">
      {weekDates.map((date,i)=>{const key=dateKey(date),items=visibleAppointments.filter(a=>a.date===key).sort((a,b)=>minutes(a.time)-minutes(b.time));return <div className={"day-column "+(selectedDate===key?"selected":"")} key={key}>
        ...
        <div className="day-list">{items.map(a=><article className={"appointment-card "+(conflictIds.has(a.id)?"appointment-conflict":"")} key={a.id}>
          <div className="appointment-time"><strong>{formatTime12(a.time)}</strong><span className="student-time">الطالب: {formatTime12(zonedParts(teacherWallClockToUtc(a.date,a.time,teacherTimezone).toISOString(),a.timezone).time)}</span></div>
          <div className="appointment-main"><h3><span className="student-country-flag">{countryFlag(a.countryCode)}</span>{a.student}</h3><p>{a.subject}</p><small>{a.country} · {a.duration} دقيقة</small></div>
    ...
    {!loading&&view==="day"&&<section className="day-view">
      ...
      {dayAppointments.length?dayAppointments.map(a=><article className={"detail-appointment "+(conflictIds.has(a.id)?"appointment-conflict":"")} key={a.id}>
        <div className="detail-time"><strong>{formatTime12(a.time)}</strong><span>{formatTime12(a.end)}</span></div><div className="detail-line"/><div className="detail-info"><h3><span className="student-country-flag">{countryFlag(a.countryCode)}</span>{a.student}</h3><p>{a.subject} · {a.country}</p><span><Clock3 size={14}/> توقيت الطالب: {formatTime12(zonedParts(teacherWallClockToUtc(a.date,a.time,teacherTimezone).toISOString(),a.timezone).time)}</span></div><span className="status">{a.status==="completed"?"مكتملة":a.status==="cancelled"?"ملغاة":a.status==="pending"?"قيد الانتظار":"مؤكد"}</span><button className="more" onClick={()=>setMenuId(menuId===a.id?null:a.id)}><MoreHorizontal size={18}/></button>{menuId===a.id&&<div className="appointment-menu"><button onClick={()=>openEdit(a)}>تعديل</button><button onClick={()=>deleteAppointment(a.id)}>إلغاء الموعد</button></div>}
